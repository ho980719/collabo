...
public class CustomJsonUsernamePasswordAuthenticationFilter extends AbstractAuthenticationProcessingFilter {
    ...
    private static final String DEFAULT_LOGIN_REQUEST_URL = "/api/v1/members/login"; // "/login"으로 오는 요청을 처리
    private static final String HTTP_METHOD = "POST"; // 로그인 HTTP 메소드는 POST
    private static final String CONTENT_TYPE = "application/json"; // JSON 타입의 데이터로 오는 로그인 요청만 처리
    private static final String USERNAME_KEY = "loginId"; // 회원 로그인 시 이메일 요청 JSON Key : "loginId"
     ...
     *    "password" : "1234"
     * }
     * 꺼낸 messageBody를 objectMapper.readValue()로 Map으로 변환 (Key : JSON의 키 -> loginId, password)
     * Map의 Key(loginId, password)로 해당 이메일, 패스워드 추출 후
